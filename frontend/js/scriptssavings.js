document.addEventListener('DOMContentLoaded', function () {
    ...
    const checkingRadioButton = document.getElementById('checking-mm-radio');
    ...
    checkingRadioButton.addEventListener('change', function () {
        if (this.checked) {
...
mainForm.addEventListener('submit', function (e) {
    ...
    // Add Savings account by default
    accounts.unshift({
        type: 'savings',
    ...
    .then(data => {
        if (data.success) {
            ...
            notification.style.display = 'block';
            notification.style.color = 'red'; // Error color
            notification.textContent = data.message;
        }
    })
    .catch(error => {
